...
/**
 * Weekly-report cron (FEAT-12). Vercel invokes this HOURLY (vercel.json —
 * needs the Pro plan; Hobby only allows daily crons and rejects deploys that
 * schedule faster). Each pass sends the report only to members whose own
 * local day + hour say it's time — so a member in Chicago who picked
 * "Sunday 5pm" gets it Sunday 5pm Chicago time. The once-per-week dedup lives
 * in the WeeklyReport ledger, so the remaining same-day passes (and any cron
 * retries) are no-ops.
 *
