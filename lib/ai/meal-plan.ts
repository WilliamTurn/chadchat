...
// delivered 172g). Per-100g macros are stored on every matched food, so the
// gap closes deterministically in code with the same offline re-scale the plan
// editor uses. No model involvement.
...
// per-macro passes need a few rounds to settle.
 ...
 * the gap, clamped to realistic portions, over a few settling rounds.
 */
function calibrateDay(day: PlanDay, target: MacroTarget): PlanDay {
...
export async function generateMealPlan({
  ...
  return {
    ...
    target: finalTarget,
    // LC-1: close the gap between the model's blind portions and the real
    // database macros so every day actually hits the target it promises.
    days: days.map((day) => calibrateDay(day, finalTarget)),
  };
}
