  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex gap-2">
        {[0, 1, 2].map((i) => (
          <div className="h-7 w-16 rounded-full bg-muted-foreground/15" key={i} />
        ))}
      </div>
      ...
      <div className="mt-5 space-y-2">
        ...
        <div className="h-3 w-11/12 rounded bg-muted-foreground/12" />
        <div className="h-3 w-4/5 rounded bg-muted-foreground/12" />
      </div>

      {/* Macro dial + bars */}
      <div className="mt-6 flex items-center gap-5">
        <div className="size-24 shrink-0 rounded-full border-8 border-muted-foreground/20" />
        <div className="flex-1 space-y-3">
          {[0, 1, 2].map((i) => (
            <div className="h-2.5 w-full rounded-full bg-muted-foreground/15" key={i} />
          ))}
        </div>
      </div>
