...
import { formatReportHour, REPORT_DAY_OPTIONS } from "@/lib/reports/schedule";
 ...
 * The controls' own state is the feedback, no success toasts (ux-canon 03
 * #32); failure renders inline (ux-canon 03 #29, #40).
 */
export function WeeklyReportSettings({
  ...
  function save(next: { enabled: boolean; day: number; hour: number }) {
    ...
    startTransition(async () => {
      try {
        ...
        setDay(prev.day);
        setHour(prev.hour);
        setError("Weekly report didn't save. Try again.");
      }
    });
  }
    ...
    <SettingsRow
      control={
        <Switch
          ...
          onCheckedChange={(next) => save({ enabled: next, day, hour })}
        />
      }
      controlId="weekly-report-enabled"
      error={error}
      label="Weekly report"
      supporting={
        <>
          A written review of your week: training, nutrition, weight trend,
          and next week's adjustments. Emailed and saved to{" "}
          <Link
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Select
              ...
              onValueChange={(v) => {
                ...
                if (nextDay !== day) {
                  save({ enabled, day: nextDay, hour });
                }
              }}
              value={String(day)}
            >
              <SelectTrigger aria-label="Report day" className="w-[9.5rem]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <Select
              ...
              onValueChange={(v) => {
                ...
                if (nextHour !== hour) {
                  save({ enabled, day, hour: nextHour });
                }
              }}
              value={String(hour)}
            >
              <SelectTrigger aria-label="Report time" className="w-[7.5rem]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {HOUR_OPTIONS.map((h) => (
